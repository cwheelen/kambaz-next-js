...
export default function PazzaPage() {
  ...
  useEffect(() => {
    ...
    client.findPostsForCourse(cid as string).then((data) => {
      dispatch(setPosts(data));
    });

    client.findFoldersForCourse(cid as string).then(async (data) => {
      if (data.length === 0) {
        const created = await Promise.all(
          DEFAULT_FOLDERS.map((name) =>
            client.createFolder(cid as string, { name })
          )
        );
        dispatch(setFolders(created));
      } else {
  ...
  const filtered = search
    ? folderFiltered.filter(
        (p: any) =>
          ...
          p.details?.replace(/<[^>]+>/g, "").toLowerCase().includes(search.toLowerCase())
      )
    : folderFiltered;
  ...
  const orderedKeys = [...groupOrder, ...extraKeys].filter((k) => groups[k]?.length);

  const selectedPost = posts.find((p: any) => p._id === selectedPostId) || null;

  const handleSelectPost = async (postId: string) => {
    dispatch(setSelectedPostId(postId));
    setShowNewPost(false);
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    try { await client.incrementPostView(postId); } catch (_) { /* ignore */ }
  };
  ...
  return (
    <div className="d-flex" style={{ height: "calc(100vh - 110px)", overflow: "hidden" }}>
      {sidebarVisible && (
        <div className="border-end d-flex flex-column" style={{ width: 320, minWidth: 280, overflowY: "auto" }}>

          <div className="d-flex border-bottom px-2 py-2"
            style={{ overflowX: "auto", gap: 6, flexWrap: "nowrap" }}>
            {folders.map((f: any) => (
              <button
                key={f._id}
                onClick={() => setActiveFolder(activeFolder === f._id ? null : f._id)}
                className={`btn btn-sm ${activeFolder === f._id ? "btn-danger" : "btn-outline-secondary"}`}
          ...
          <div className="d-flex align-items-center px-2 py-2 border-bottom gap-2">
            <button className="btn btn-sm btn-outline-secondary"
              onClick={() => setSidebarVisible(false)} title="Hide sidebar">◀</button>
            <button className="btn btn-sm btn-danger"
              onClick={() => { setShowNewPost(true); dispatch(setSelectedPostId(null)); }}>
              + New Post
            </button>
            <input type="search" className="form-control form-control-sm"
              placeholder="Search or add a post..." value={search}
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {orderedKeys.map((groupKey) => (
              <div key={groupKey}>
                <div
                  ...
                  style={{ backgroundColor: "#f5f5f5", cursor: "pointer", fontSize: 12, fontWeight: 600 }}
                  onClick={() => setCollapsed((c) => ({ ...c, [groupKey]: !c[groupKey] }))}
                >
                  <span>▾ {groupKey}</span>
                </div>
                {!collapsed[groupKey] && groups[groupKey].map((post: any) => (
                  <div
                    key={post._id}
                  ...
                  >
                    <div className="d-flex justify-content-between align-items-start">
                      <div className="fw-bold" style={{ fontSize: 13, lineHeight: 1.3 }}>
                        {post.summary}
                      </div>
                      <span className="text-muted ms-2" style={{ fontSize: 11, whiteSpace: "nowrap" }}>
                        {formatTime(post.createdAt)}
                      </span>
                    </div>
                    <div className="text-muted" style={{ fontSize: 12, marginTop: 2 }}>
                      {post.authorRole === "FACULTY" || post.authorRole === "ADMIN" ? "Instr" : "Student"}
                      {" "}· {post.authorName}
                    </div>
                    <div className="text-secondary" style={{
                      fontSize: 12, overflow: "hidden", display: "-webkit-box",
            ...
            {sorted.length === 0 && (
              <div className="text-center text-muted p-4" style={{ fontSize: 13 }}>No posts yet.</div>
            )}
          </div>
        </div>
      )}
      ...
      {!sidebarVisible && (
        <div className="d-flex align-items-start pt-3 ps-2 border-end">
          <button className="btn btn-sm btn-outline-secondary"
            onClick={() => setSidebarVisible(true)} title="Show sidebar">▶</button>
        </div>
      )}
